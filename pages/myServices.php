...
if(isset($developerID)) {
  addDeveloperBlogs($developerID); 
}
function addDeveloperBlogs($developerID)
{
  include('config-DB.php');
  //Edited by Arwa
  $sql = "SELECT  `developerId`, `category`, `description`, `price`, `date` FROM `service` WHERE `developerId`=".$developerID." ORDER BY date(date) DESC";
  $result = $conn->query($sql);

  if ($result->num_rows > 0) {
    // output data of each row
    while ($row = $result->fetch_assoc()) {
      if (strcasecmp($row["category"], "UI") == 0) {
        addUI($row["description"], $row["price"], $row["date"]);
      } elseif (strcasecmp($row["category"], "mobile") == 0) {
        addMobile($row["description"], $row["price"], $row["date"]);
      } elseif (strcasecmp($row["category"], "web") == 0) {
